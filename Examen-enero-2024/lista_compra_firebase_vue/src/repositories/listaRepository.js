...
// MODIFICADO: Se expone esta función (ya estaba), no hay cambios en su interior
...
// NUEVO: Para ver los detalles de un item
async function getItemById(id) {
    if (!auth.currentUser) throw new Error("usuario no autentificado")
    let itemRef = doc(db, "items", id)
    let itemSnapshot = await getDoc(itemRef)
    if (!itemSnapshot.exists()) {
        throw new Error("El item no existe")
    }
    if (itemSnapshot.data().uid !== auth.currentUser.uid) {
